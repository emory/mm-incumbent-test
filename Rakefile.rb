...
# Usage: rake bucket
desc "s3_website push to shove content to Amazon S3"
task :bucket do
  puts "* shoveling the contents of ./_site into the incumbent.org bucket at AWS"
...
namespace :deploy do
  desc "Regenerate and push production files and notify services of the update"
  task :prod => ["build", "bucket", "notify"] do
  end
